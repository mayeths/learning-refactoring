...
「重新回顾」这个重构手法对于异常处理的设计来说很是关键。一个函数对对象池进行查找，并返回一个指针，如果找不到，你会怎么做？通常的做法是返回一个 `null`，这是没问题的。如果返回一个对象，你该怎么办呢？用不了 `null`，你应该考虑增加一个特例对象——类似于 `UnknownObject`。这样，你可以通过判断类型来知晓是否出现了 failure。
